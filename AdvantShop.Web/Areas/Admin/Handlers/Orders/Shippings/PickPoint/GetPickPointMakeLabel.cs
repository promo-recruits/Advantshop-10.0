...
{
    public class GetPickPointMakeLabel
    {
        private readonly int _orderId;
        public List<string> Errors { get; set; }

        public GetPickPointMakeLabel(int orderId)
        {
            _orderId = orderId;
            ...
            if (!string.IsNullOrEmpty(orderAdditionalData))
            {
                ...
                if (order != null)
                {
                                ...
                                typeof(ShippingKeyAttribute), false).First()).Value)
                    {
                        ...
                        var fullFilePath = pickPointMethod.PickPointApiService.GetMakeLabel(orderAdditionalData, path);
