...
namespace AdvantShop.App.Landing.Handlers.Landings
{
    public class UploadAttachmentsHandler
    {
        ...
        public List<UploadAttachmentsResult> Execute<T>(List<HttpPostedFile> files) where T : Attachment, new()
        {
            var result = new List<UploadAttachmentsResult>();
            
            foreach (var file in files)
                result.Add(AddAttachment<T>(file));
        ...
        public UploadAttachmentsResult[] Validate<T>() where T : Attachment, new()
        {
            if (HttpContext.Current == null || HttpContext.Current.Request.Files.Count == 0)
                return new[]
                {
                    new UploadAttachmentsResult
                        {Result = false, Error = LocalizationService.GetResource("Admin.Attachments.FileNotFound")}
                };
        ...
        private UploadAttachmentsResult ValidateFile<T>(HttpPostedFile file, AttachmentType type, ref int filesLength)
            where T : Attachment, new()
        {
            if (file != null && file.ContentLength > 0)
            {
                ...
                return new UploadAttachmentsResult
                    {Result = true, Attachment = new AttachmentModel {FileName = fileName}};
            }
