...
namespace Tools.core
{
    public partial class CleanUp : Page
    {
        protected void btnCleanUpPictureFolder_Click(object sender, EventArgs e)
        {
            ...
            if (Directory.Exists(FoldersHelper.GetImageProductPathAbsolut(ProductImageType.Big, string.Empty)))
            {
                files.AddRange(Directory.GetFiles(FoldersHelper.GetImageProductPathAbsolut(ProductImageType.Big, string.Empty)));
            }
            if (Directory.Exists(FoldersHelper.GetImageProductPathAbsolut(ProductImageType.Middle, string.Empty)))
            {
                files.AddRange(Directory.GetFiles(FoldersHelper.GetImageProductPathAbsolut(ProductImageType.Middle, string.Empty)));
            }
            if (Directory.Exists(FoldersHelper.GetImageProductPathAbsolut(ProductImageType.Small, string.Empty)))
            {
                files.AddRange(Directory.GetFiles(FoldersHelper.GetImageProductPathAbsolut(ProductImageType.Small, string.Empty)));
            }
            if (Directory.Exists(FoldersHelper.GetImageProductPathAbsolut(ProductImageType.XSmall, string.Empty)))
            {
                files.AddRange(Directory.GetFiles(FoldersHelper.GetImageProductPathAbsolut(ProductImageType.XSmall, string.Empty)));
            }
            // CategoryImageType ------------------------

            if (Directory.Exists(FoldersHelper.GetImageCategoryPathAbsolut(CategoryImageType.Big, string.Empty)))
            {
            ...
            if (Directory.Exists(FoldersHelper.GetPathAbsolut(FolderType.BrandLogo, string.Empty)))
            {
                files.AddRange(Directory.GetFiles(FoldersHelper.GetPathAbsolut(FolderType.BrandLogo, string.Empty)));
            }
            if (Directory.Exists(FoldersHelper.GetPathAbsolut(FolderType.Carousel, string.Empty)))
            {
                files.AddRange(Directory.GetFiles(FoldersHelper.GetPathAbsolut(FolderType.Carousel, string.Empty)));
            }
            if (Directory.Exists(FoldersHelper.GetPathAbsolut(FolderType.MenuIcons, string.Empty)))
            {
                files.AddRange(Directory.GetFiles(FoldersHelper.GetPathAbsolut(FolderType.MenuIcons, string.Empty)));
            }
            if (Directory.Exists(FoldersHelper.GetPathAbsolut(FolderType.News, string.Empty)))
            {
            ...
            {
                if (File.Exists(file) && (!fileNames.Contains(file) || (file.Trim().Length == 0)))
                {
