...
namespace AdvantShop.Core.Services.Crm.Vk.VkMarket.Export
{
    public static class VkMarketExportState
    {
        ...
        private const string FileLogPath = "~/content/vkmarket/";
        ...
        public static void Start()
        {
            ...
            {
                _isRun = true;
                _fileLog = GetDirectory() + "report_" + DateTime.Now.ToString("yy-MM-dd_hh-mm") + ".txt";
        ...
        public static void DeleteExpiredLogs()
        {
            ...
            var reports = dirInfo.GetFiles("*.txt").OrderByDescending(x => x.CreationTime).ToList();
            if (reports.Count > 10)
            {
        ...
        public static List<string> GetReports()
        {
            var dir = GetDirectory();
            var dirInfo = new DirectoryInfo(dir);
            var reports = dirInfo.GetFiles("*.txt").OrderByDescending(x => x.CreationTime).Take(10).Select(x => x.Name).ToList();
