...
namespace AdvantShop.Helpers
{
    public class MobileHelper
    {
        ...
        public static bool IsMobileBrowser()
        {
            ...
            var isMobile = (MobileRegex1.IsMatch(ua) || MobileRegex2.IsMatch(ua.Substring(0, Math.Min(ua.Length, 4)))) && !Tablets.Any(currnetUserAgent.Contains);
        ...
        {
            var mode = CommonHelper.GetCookie("deviceMode");

            var result = mode != null && mode.Value == "desktop" &&
                         HttpContext.Current.Items["_ForcedMobile"] == null;

            if (!result && HttpContext.Current.Request.QueryString["deviceMode"] == "desktop")
            {
                SetDesktopForcedCookie();
        ...
        {
            var mode = HttpContext.Current.Request.QueryString["deviceMode"];

            if ((!string.IsNullOrEmpty(mode) && mode == "desktop") || !SettingsMobile.IsMobileTemplateActive)
            {
                SetDesktopForcedCookie();
                return false;
            }
            ...
            var result = forcedCookie != null && forcedCookie.Value == "mobile";

            if (!result && (HttpContext.Current.Request.QueryString["deviceMode"] == "mobile" || HttpContext.Current.Request.QueryString["MobilePreview"] == "true"))
            {
                if(HttpContext.Current.Request.QueryString["deviceMode"] == "mobile")
        ...
        {
            CommonHelper.SetCookie("deviceMode", "desktop", false);
        }

        public static void SetMobileForcedCookie()
        {
            CommonHelper.SetCookie("deviceMode", "mobile", false);
        }
